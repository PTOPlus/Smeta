...
def add_clipboard_support(widget):
    def copy(event):
        try:
            ...
            event.widget.clipboard_clear(); event.widget.clipboard_append(text)
        except tk.TclError: pass
        return "break"
    def paste(event):
        try: event.widget.insert(tk.INSERT, event.widget.clipboard_get())
        except tk.TclError: pass
        return "break"
    widget.bind("<Control-c>", copy); widget.bind("<Control-v>", paste)
    widget.bind("<Command-c>", copy); widget.bind("<Command-v>", paste)

def add_context_menu(widget):
    menu = tk.Menu(widget, tearoff=0)
    def cut():
        try:
            text = widget.selection_get(); widget.clipboard_clear(); widget.clipboard_append(text)
            widget.delete(tk.SEL_FIRST, tk.SEL_LAST)
        except tk.TclError: pass
    def copy():
        try: text = widget.selection_get(); widget.clipboard_clear(); widget.clipboard_append(text)
        except tk.TclError: pass
    def paste():
    ...
    def show_menu(event):
        try: has_selection = widget.tag_ranges(tk.SEL)
        except Exception: has_selection = False
        ...
        try: widget.clipboard_get(); menu.entryconfigure(2, state="normal")
        except tk.TclError: menu.entryconfigure(2, state="disabled")
...
class SmetaApp:
    def __init__(self, root):
        ...
        self.tab_calc = tk.Frame(self.notebook); self.tab_db = tk.Frame(self.notebook)
    ...
    def setup_db_tab(self):
        ...
        self.db_combo = ttk.Combobox(db_sel_frame, width=40, state="readonly"); self.db_combo.pack(side=tk.LEFT, padx=5)
        ...
        add_clipboard_support(self.work_text); add_context_menu(self.work_text)
        ...
        e = tk.Entry(frame_input, width=18); add_clipboard_support(e); add_context_menu(e); e.grid(row=1, column=1, padx=5, pady=2, sticky="w"); self.entries['Ед_изм_раб'] = e
        tk.Label(frame_input, text="Материал:").grid(row=1, column=2, sticky="w", pady=2)
        e = tk.Entry(frame_input, width=40); add_clipboard_support(e); add_context_menu(e); e.grid(row=1, column=3, padx=5, pady=2, sticky="w"); self.entries['Материал'] = e
        tk.Label(frame_input, text="Ед. изм. материала:").grid(row=2, column=0, sticky="w", pady=2)
        e = tk.Entry(frame_input, width=18); add_clipboard_support(e); add_context_menu(e); e.grid(row=2, column=1, padx=5, pady=2, sticky="w"); self.entries['Ед_изм'] = e
        frame_v1 = tk.LabelFrame(frame_input, text="Вариант 1", padx=8, pady=6); frame_v1.grid(row=3, column=0, columnspan=2, padx=5, pady=10, sticky="we")
        frame_v2 = tk.LabelFrame(frame_input, text="Вариант 2", padx=8, pady=6); frame_v2.grid(row=3, column=2, columnspan=2, padx=5, pady=10, sticky="we")
        ...
        for frame, suffix in ((frame_v1, "_1"), (frame_v2, "_2")):
            for r, (label, base) in enumerate(variant_fields):
                ...
                en = tk.Entry(frame, width=16); add_clipboard_support(en); add_context_menu(en); en.grid(row=r, column=1, padx=5, pady=2); self.entries[key] = en
        btn_f = tk.Frame(self.tab_db); btn_f.pack(pady=5)
        tk.Button(btn_f, text="Сохранить в базу", bg="#4CAF50", fg="white", command=self.save_to_db).pack(side=tk.LEFT, padx=5)
        tk.Button(btn_f, text="Удалить из базы", bg="#f44336", fg="white", command=self.delete_from_db).pack(side=tk.LEFT, padx=5)
        filter_frame = tk.Frame(self.tab_db); filter_frame.pack(fill=tk.X, padx=10, pady=5)
        tk.Label(filter_frame, text="🔍 Поиск работы:").pack(side=tk.LEFT)
        self.filter_work = tk.Entry(filter_frame, width=35); self.filter_work.pack(side=tk.LEFT, padx=5)
        tk.Label(filter_frame, text="🔍 Поиск материала:").pack(side=tk.LEFT, padx=(10, 0))
        self.filter_mat = tk.Entry(filter_frame, width=35); self.filter_mat.pack(side=tk.LEFT, padx=5)
    ...
    def clear_db_filters(self):
        self.filter_work.delete(0, tk.END); self.filter_mat.delete(0, tk.END); self.refresh_db_table()
    ...
    def save_to_db(self):
        try:
            ...
            for c in ('Ед_изм_раб', 'Материал', 'Ед_изм'): data[c] = self.entries[c].get().strip()
            if not data['Работа']: return messagebox.showerror("Ошибка", "Введите наименование работы.")
            if data['Материал'] in ("", "-", "0"): data['Материал'] = "-"
    ...
    def refresh_db_table(self):
        ...
        if work_q: df = df[df['Работа'].astype(str).str.lower().str.contains(work_q, na=False)]
        if mat_q: df = df[df['Материал'].astype(str).str.lower().str.contains(mat_q, na=False)]
    ...
    def load_to_entries(self, event):
        sel = self.tree_db.selection()
        if not sel: return
        try: idx = int(sel[0]); row = self.db.loc[idx]
        except (KeyError, ValueError): return
        self.work_text.delete("1.0", tk.END); self.work_text.insert("1.0", row['Работа'])
    ...
    def delete_current_db(self):
        ...
        except Exception as e: messagebox.showerror("Ошибка", f"Не удалось удалить файл:\n{e}")

    def setup_calc_tab(self):
        frame_title = tk.Frame(self.tab_calc, pady=4); frame_title.pack(fill=tk.X, padx=10)
        tk.Label(frame_title, text="Наименование сметы / объект:").pack(side=tk.LEFT)
        self.title_entry = tk.Entry(frame_title); add_clipboard_support(self.title_entry); add_context_menu(self.title_entry)
        self.title_entry.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        frame_top = tk.Frame(self.tab_calc, pady=5); frame_top.pack(fill=tk.X, padx=10)
        self.work_combo = ttk.Combobox(frame_top, width=120); self.work_combo.pack(side=tk.LEFT, padx=5)
        self.work_combo.bind('<KeyRelease>', self.filter_works_combo); self.update_combobox()
        tk.Label(frame_top, text="Объём:").pack(side=tk.LEFT)
        self.vol_entry = tk.Entry(frame_top, width=10); add_clipboard_support(self.vol_entry); add_context_menu(self.vol_entry)
        self.vol_entry.pack(side=tk.LEFT, padx=5)
        tk.Button(frame_top, text="Добавить работу", bg="#2196F3", fg="white", command=self.add_to_estimate).pack(side=tk.LEFT, padx=10)
        frame_tools = tk.Frame(self.tab_calc, pady=5); frame_tools.pack(fill=tk.X, padx=10)
        tk.Button(frame_tools, text="➕ Раздел", bg="#9C27B0", fg="white", command=self.add_section).pack(side=tk.LEFT, padx=5)
    ...
    def _editable_cols_for(self, name_raw):
        ...
        if sc.is_work(name_raw): return {1, 4, 5, 9}
        if sc.is_material(name_raw): return {1, 3, 5, 7, 9}
        return set()
    ...
    def on_tree_double_click(self, event):
        ...
        if sc.is_section(name_raw) and col_idx == 1: self._edit_section_inline(item, vals); return
        if col_idx in self._editable_cols_for(name_raw): self._start_cell_edit(item, vals, col_idx)

    def add_to_estimate(self):
        work_name = self.work_combo.get().strip()
        if not work_name: return messagebox.showwarning("Внимание", "Выберите работу из списка.")
        raw = self.vol_entry.get().strip()
        try: vol = float(raw.replace(',', '.'))
        except ValueError: return messagebox.showerror("Ошибка", "Введите корректное число в поле «Объём».")
    ...
    def remove_smeta_row(self):
        ...
        for item in selected: self.tree_smeta.delete(item)
        self.full_rebuild()
    ...
    def _edit_section_inline(self, item, vals):
        ...
        add_clipboard_support(self.edit_entry); add_context_menu(self.edit_entry)
        self.edit_entry.insert(0, vals[1].replace("РАЗДЕЛ: ", "")); self.edit_entry.focus(); self.edit_entry.select_range(0, tk.END)
    ...
    def _start_cell_edit(self, item, vals, col_idx):
        ...
        self.edit_entry.place(x=root_x + x, y=root_y + y, width=w, height=h)
        add_clipboard_support(self.edit_entry); add_context_menu(self.edit_entry)
        self.edit_entry.insert(0, str(vals[col_idx])); self.edit_entry.focus(); self.edit_entry.select_range(0, tk.END)
    ...
    def _finish_cell_edit(self):
        ...
        if nv == "": return self._destroy_edit()
        nvals = self.edit_orig[:]
        numeric_cols = {3, 4, 5, 7, 9}
        if self.edit_col_idx in numeric_cols:
            try: nvals[self.edit_col_idx] = float(nv.replace(',', '.'))
            except ValueError: messagebox.showerror("Ошибка", "Введите корректное число."); self.edit_entry.focus(); return
        ...
        try: self.full_rebuild()
        except Exception as e: messagebox.showerror("Ошибка пересчёта", f"Не удалось обновить итоги:\n{e}")
        finally: self._destroy_edit()
    ...
    def _open_material_dialog(self, initial_data=None):
        ...
        for i, (lbl, key, w) in enumerate(fields):
            ...
            e = tk.Entry(win, width=w); add_clipboard_support(e); add_context_menu(e)
            if initial_data and key in initial_data: e.insert(0, str(initial_data[key]))
            e.grid(row=i, column=1, padx=5, pady=4, sticky="w"); entries[key] = e
        err_label = tk.Label(win, text=" ", fg="red"); err_label.grid(row=len(fields), column=0, columnspan=2)
        def apply():
            name = entries['name'].get().strip()
            if not name: err_label.config(text="Введите наименование материала."); return
            data = {'name': name, 'unit': entries['unit'].get().strip()}
            for key in ('norm1', 'price1', 'norm2', 'price2'):
                raw = entries[key].get().strip()
                if raw == "": data[key] = 0.0; continue
                try: data[key] = float(raw.replace(',', '.'))
                except ValueError: err_label.config(text=f"Поле «{key}» должно быть числом."); return
            result.update(data); win.destroy()
        def cancel(): result.clear(); win.destroy()
        btn_f = tk.Frame(win); btn_f.grid(row=len(fields) + 1, column=0, columnspan=2, pady=10)
        ...
        win.bind("<Return>", lambda e: apply()); win.wait_window()
        return result if result else None

    def add_material_to_selected(self):
        sel = self.tree_smeta.selection()
        if not sel: return messagebox.showwarning("Внимание", "Выберите работу или материал.")
        item = sel[0]; vals = self.tree_smeta.item(item, 'values'); name_raw = str(vals[1]).strip()
        if sc.is_section(name_raw): return messagebox.showwarning("Внимание", "Нельзя добавить материал в раздел. Выберите работу.")
        if sc.is_total(name_raw): return messagebox.showwarning("Внимание", "Выберите строку работы или материала, не итоговую строку.")
    ...
    def load_estimate(self):
        ...
        try: xl = pd.ExcelFile(file_path)
        except Exception as e: return messagebox.showerror("Ошибка", f"Не удалось открыть файл:\n{e}")
        meta_df = None
        if "Meta" in xl.sheet_names: meta_df = pd.read_excel(xl, sheet_name="Meta")
        else:
        ...
        if "Смета" not in xl.sheet_names: return messagebox.showerror("Ошибка", "В файле отсутствует лист 'Смета'.")
        smeta_df = pd.read_excel(xl, sheet_name="Смета", header=None)
        if meta_df is not None and not meta_df.empty: self._reconcile_meta(meta_df)
        res = sc.parse_exported_sheet(smeta_df.values.tolist())
        self.tree_smeta.delete(*self.tree_smeta.get_children())
        self.title_entry.delete(0, tk.END)
        if res['title']: self.title_entry.insert(0, res['title'])
        missing = []
    ...
    def _reconcile_meta(self, meta_df):
        for col in sc.COLS:
            if col not in meta_df.columns: meta_df[col] = "-" if col in ('Работа', 'Ед_изм_раб', 'Материал', 'Ед_изм') else 0.0
        numeric_cols = ('Расход_1', 'Цена_мат_1', 'Цена_раб_1', 'Расход_2', 'Цена_мат_2', 'Цена_раб_2')
        new_rows_list = []
        for _, meta_row in meta_df.iterrows():
            work = str(meta_row['Работа']).strip(); material = str(meta_row['Материал']).strip()
            existing = self.db[(self.db['Работа'].astype(str).str.strip() == work) & (self.db['Материал'].astype(str).str.strip() == material)]
            if existing.empty: new_rows_list.append(meta_row); continue
            match_found = False
            for _, exist_row in existing.iterrows():
                try:
                    ...
                    for nc in numeric_cols: ok = ok and abs(sc.to_float(exist_row[nc]) - sc.to_float(meta_row[nc])) < 1e-6
                    if ok: match_found = True; break
                except Exception: continue
            if not match_found: new_rows_list.append(meta_row)
        if new_rows_list:
    ...
    def export_excel(self):
        rows = self._gather_rows()
        if not rows: return messagebox.showwarning("Внимание", "Смета пуста — нечего выгружать.")
        try:
            ...
            for vals in rows:
                ...
                if sc.is_work(name): seen_works.add(sc.clean_name(name))
                elif sc.is_material(name): seen_materials.add(sc.clean_name(name))
            meta_rows = []
    ...
    def undo_action(self, event=None):
        if self.undo_stack:
            ...
            if self.tree_smeta.exists(item_id):
                ...
                vals[col_idx] = old_val; self.tree_smeta.item(item_id, values=tuple(vals))
                self.full_rebuild()
    ...
    def _copy_cell(self):
        if self.ctx_menu_item:
            ...
            self.root.clipboard_clear(); self.root.clipboard_append(str(val))

    def _paste_cell(self):
        if not self.ctx_menu_item: return
        try: clip = self.root.clipboard_get().strip()
        except tk.TclError: return
        col_idx = int(self.ctx_menu_col[1:]) - 1
        vals = self.tree_smeta.item(self.ctx_menu_item, 'values')
        name_raw = str(vals[1]).strip()
        if col_idx not in self._editable_cols_for(name_raw): return
        self._start_cell_edit(self.ctx_menu_item, vals, col_idx)
        if self.edit_entry and self.edit_entry.winfo_exists():
            self.edit_entry.delete(0, tk.END); self.edit_entry.insert(0, clip); self.edit_entry.icursor(tk.END)
