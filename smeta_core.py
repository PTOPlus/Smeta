...
def to_float(val, default=0.0):
    """Надёжно парсит число из любого типа, поддерживает запятую как десятичный разделитель.
    
    Преобразует значение в float, обрабатывая None, NaN, пустые строки и специальные
    токены ('-', '0', 'nan', 'none'). Заменяет запятую на точку для поддержки
    русской локали.
    
    Args:
        val: значение для преобразования (str, int, float, None).
        default (float): значение по умолчанию при некорректном вводе.
...
def is_section(name) -> bool:
    """Проверяет, является ли строка разделом сметы.
    
    Args:
        name: название строки для проверки.
        
    Returns:
        bool: True если строка начинается с префикса 'РАЗДЕЛ: '.
    """
    return str(name).strip().startswith(SECTION_PREFIX)

def is_work(name) -> bool:
    """Проверяет, является ли строка работой в смете.
    
    Args:
        name: название строки для проверки.
        
    Returns:
        bool: True если строка начинается с префикса 'Работа:'.
    """
    return str(name).strip().startswith("Работа:")

def is_total(name) -> bool:
    """Проверяет, является ли строка итоговой (ИТОГО ПО УЗЛУ).
    
    Args:
        name: название строки для проверки.
        
    Returns:
        bool: True если строка начинается с префикса 'ИТОГО ПО УЗЛУ: '.
    """
    return str(name).strip().startswith(TOTAL_PREFIX)

def is_material(name) -> bool:
    """Проверяет, является ли строка материалом в смете.
    
    Args:
        name: название строки для проверки.
        
    Returns:
        bool: True если строка начинается с символа '>'.
    """
...
def clean_name(name) -> str:
    ...
    Args:
        name: название с префиксом.
        
    Returns:
        str: очищенное название без префиксов и лишних пробелов.
    """
    s = str(name).strip()
...
def migrate_legacy_df(df):
    """Мигрирует DataFrame из старого формата в новый с двумя вариантами цен.
    
    Если в DataFrame есть только legacy-колонки (LEGACY_COLS), создаёт новые
    колонки '_1' и '_2', копируя значения из legacy-колонок. Если формат уже новый,
    дополняет отсутствующие колонки значениями по умолчанию.
    
    Args:
        df (pd.DataFrame): DataFrame для миграции.
        
    Returns:
        pd.DataFrame: DataFrame с колонками COLS (двухвариантный формат).
    """
...
def build_work_block(work_name, vol, db, next_num, db_manager=None):
    """Строит блок строк сметы для одной работы: строка работы + материалы + строка ИТОГО.
    
    Испольует либо нормализованную БД (через db_manager), либо legacy DataFrame.
    Рассчитывает стоимости материалов и работы с учётом объёма и норм расхода.
    
    Args:
        work_name (str): название работы из справочника.
        vol (float): объём работы.
        db (pd.DataFrame): DataFrame с данными справочника (legacy-формат).
        next_num (int): следующий номер строки.
        db_manager (DatabaseManager, optional): менеджер БД для работы с SQLite.
        
    Returns:
        list или None: список кортежей строк (работа, материалы, итог),
            или None если работа не найдена в базе.
    """
...
def rebuild_smeta(rows):
    """Полная пересборка сметы — единая точка пересчёта всех значений.
    
    Проходит по всем строкам сметы и:
    - Пересчитывает объёмы материалов как Норма × Объём работы
    - Пересчитывает стоимости как Объём × Цена
    - Формирует строки ИТОГО ПО УЗЛУ после каждого блока работы
    - Сохраняет разделы без изменений
    
    Важно: объёмы материалов никогда не вводятся пользователем вручную —
    они всегда вычисляются как произведение нормы расхода на объём работы.
    Это исправляет баг, при котором изменение объёма работы не пересчитывало
    объёмы материалов, оставляя устаревшие значения.
    
    Args:
        rows (list): список кортежей строк сметы (без строки ИТОГО).
        
    Returns:
        list: полностью пересчитанный список строк сметы с итогами.
    """
...
def compute_grand_totals(rows):
    ...
    Returns:
        tuple: (итого_в1, итого_в2) — общая сумма варианта 1 и варианта 2.
    """
    t1 = t2 = 0.0
    for vals in rows:
        name = str(vals[1]).strip()
...
def export_smeta_to_excel(rows, output_path, title="", meta_rows=None,
    ...
    с автоформулами для расчёта стоимостей. При наличии meta_rows создаёт
    дополнительный лист 'Meta' с метаданными для восстановления справочника.
